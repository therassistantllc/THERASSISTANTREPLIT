...
export async function POST(request: Request) {
  try {
    ...
    if (!raw835.includes("ISA") || !raw835.includes("CLP")) {
      return NextResponse.json(
        { success: false, error: "Availity ERA response does not appear to be a valid 835 file." },
        { status: 422 },
      );
    }
    ...
    const { error: batchError } = await supabase.from("payment_import_batches").insert({
      ...
      import_source: "835_era_availity",
    ...
    for (const [claimIndex, claim] of parsed.claims.entries()) {
      ...
      if (itemRecord.posting_ready) {
        const { error: queueError } = await supabase.from("workqueue_items").insert({
          ...
          description: `${claim.payerName ?? "Payer"} - $${Number(claim.paidAmount ?? 0).toFixed(2)} (auto-ingested from Availity)`,
          resolved_at: null,
          created_at: now,
          updated_at: now,
        });
