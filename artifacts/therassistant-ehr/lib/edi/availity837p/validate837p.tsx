...
export function validateAvaility837PClaim(
  ...
  const errors: Availity837PValidationError[] = [];
  const warnings: Availity837PValidationError[] = [];
  ...
  } else if (connection.receiver_id !== "030240928") {
    pushWarning(
      warnings,
      "connection.receiver_id",
      'Availity Batch EDI Companion Guide specifies receiver_id "030240928" (Availity Dun & Bradstreet number) for ISA08.',
      "ISA/GS",
      "ISA",
    );
  }
  ...
  if (!isNonEmptyString(connection.receiver_name)) {
    ...
    // Availity Batch EDI Companion Guide specifies the literal value "Availity"
    // for Loop 1000B NM103. Accept any casing of "Availity" silently; warn
    // loudly on legacy Office Ally values so stale connections surface in audit.
    ...
    if (normalized === "OFFICE ALLY" || normalized === "OFFICEALLY") {
      pushWarning(
        ...
        'Legacy Office Ally receiver_name detected — update to "Availity" per Availity Batch EDI Companion Guide (Loop 1000B NM103).',
        "ISA/GS",
        "1000B",
      );
    } else if (normalized !== "AVAILITY") {
      pushWarning(
        warnings,
        "connection.receiver_name",
        'Using an explicit Availity receiver_name override; the Companion Guide specifies "Availity".',
        "ISA/GS",
        "1000B",
      );
    }
  ...
  if (!isNonEmptyString(payerProfile.availity_payer_id)) {
    pushError(errors, "payerProfile.availity_payer_id", "payer_id is required.", "2010BB", "NM1");
  }
  // Availity payer IDs are variable-length alphanumeric (e.g. blue-shield IDs
  // like "BS001", CMS IDs like "12B45"); no length warning.
